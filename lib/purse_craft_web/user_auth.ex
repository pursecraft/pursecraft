defmodule PurseCraftWeb.UserAuth do
  ...
  # Make the remember me cookie valid for 14 days. This should match the session validity setting in UserToken.
  ...
  def log_out_user(conn) do
    ...
    if live_socket_id = get_session(conn, :live_socket_id) do
      PurseCraftWeb.Endpoint.broadcast(live_socket_id, "disconnect", %{})
    end
  ...
  the `current_scope`:
      ...
      defmodule PurseCraftWeb.PageLive do
        use PurseCraftWeb, :live_view

        on_mount {PurseCraftWeb.UserAuth, :mount_current_scope}
        ...
      end

  Or use the `live_session` of your router to invoke the on_mount callback:

      live_session :authenticated, on_mount: [{PurseCraftWeb.UserAuth, :require_authenticated}] do
  ...
  def on_mount(:require_sudo_mode, _params, session, socket) do
    ...
    if Identity.sudo_mode?(socket.assigns.current_scope.user, -10) do
      {:cont, socket}
    else
      socket =
        socket
