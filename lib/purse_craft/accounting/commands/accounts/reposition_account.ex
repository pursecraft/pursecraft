defmodule PurseCraft.Accounting.Commands.Accounts.RepositionAccount do
  ...
  def call(%Scope{} = scope, account_id, prev_account_id, next_account_id) do
    ...
    accounts = AccountRepository.list_by_external_ids(ids_to_fetch, preload: [:book])
    accounts_map = Map.new(accounts, &{&1.external_id, &1})
    ...
    with {:ok, [account, prev_account, next_account]} <-
      ...
      case result do
        {:ok, updated} ->
          BroadcastBook.call(account.book, {:account_repositioned, updated})
          {:ok, updated}
  ...
  defp validate_and_extract_accounts(accounts_map, account_id, prev_account_id, next_account_id) do
    case Map.get(accounts_map, account_id) do
      ...
      account ->
        with {:ok, prev_account} <- validate_optional_account(accounts_map, prev_account_id, account.book_id),
